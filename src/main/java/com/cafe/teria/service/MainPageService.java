...
public class MainPageService {
	...
	@Autowired
	com.cafe.teria.dao.MainPageDAO dao;

	private Logger logger = LoggerFactory.getLogger(this.getClass());

	public ArrayList<com.cafe.teria.dto.MainPageDTO> mainSearch(String keyword, int category, String[] filterValues) {
		logger.info(keyword);
		...
		/*
		 * 날짜는 자바 8버전 이전에선 calender와 Date를 사용 8 버전 이후로는 LocalDate / LocalDateTime 를 사용
		...
		switch (category) {
		case 1:
			searchSort = "cafe_likecnt";
			break;
		case 2:
			searchSort = "cafe_hit";
			break;
		case 3:
			searchSort = "cafe_ratestaravg";
			break;
		}
		
		logger.info("searchSort : {}", searchSort);
		...
		logger.info("쿼리 결과 갯수 : {}", result.size());
		  ...
		  for (MainPageDTO mainPageDTO : result) { 
			  logger.info("여기서 확인 식당명 : {}",mainPageDTO.getCafe_title()); 
			  logger.info("날짜 : {}", mainPageDTO.getDiet_date()); 
			  logger.info("끼니구분 : {}", mainPageDTO.getDiet_code()); 
			  logger.info("좋아요 수 : {}",mainPageDTO.getCafe_likecnt()); 
			  logger.info("조회수 : {}", mainPageDTO.getCafe_hit());
			  logger.info("별점수 : {}", mainPageDTO.getCafe_ratestaravg());
		  }
		...
		if (diteCode == 2) { // 해당 날짜의 아침을 제외
			...
			/*
			 * int cnt = 0; 보통 for문 돌리는 것처럼 cnt를 두고 번호로 remove 하려고 했는데.. list를 이렇게 하면 지워진
			 * index가 바로 처리가 되기 때문에 끝에서 빈공간이 발생하는 듯 list 삭제는 iterator 사용하기
			...
			while (iter.hasNext()) {
				...
				if (mainPageDTO.getDiet_date() != null) {
					...
					if (mainPageDTO.getDiet_date().compareTo(startDateSql) == 0 & mainPageDTO.getDiet_code() == 1) {
		...
		if (filterValues != null) {
			if (filterValues.length > 0) {
				...
				filterValues = Arrays.copyOfRange(filterValues, Arrays.asList(filterValues).indexOf("/") + 1,
						filterValues.length);
				...
				while (iter.hasNext()) {
					...
					// if(mainPageDTO.getDiet_date_name() != null | mainPageDTO.getDiet_code() != 0)
					// { // code는 쿼리결과가 null 이면 0으로 떨어짐

					// 선택한 필터의 요일과 비교해보고, 쿼리 결과값에 해당 요일 or 끼니가 없다면
					// logger.info("요일 boolean : {}",
					// Arrays.asList(filterValues).contains(mainPageDTO.getDiet_date_name()));
					// logger.info("끼니 boolean : {}",
					// Arrays.asList(filterValues).contains(Integer.toString(mainPageDTO.getDiet_code())));
					...
					} else if (filterCategory == 14 | filterCategory == 13 | filterCategory == 11
							| filterCategory == 7) {
						// 세개의 필터를 선택한 경우
				...
				/*
				 * 끼니 필터 => 위 요일 필터에 병합 // 끼니가 비어있지 않으면 => db select 에서 (식당명이 메뉴명이랑 같을 경우 ) null
				 * 값이 여기선 0이 된다 if(mainPageDTO.getDiet_code() != 0) {
				 * if(Arrays.asList(filterValues).contains(mainPageDTO.getDiet_code())){ // 찍힌
				 * 요일만 남기고 다른 데이터는 지우기 logger.info("끼니 필터에 의해 선택된 값 : {}",
				 * mainPageDTO.getDiet_code()); }else { logger.info("끼니 필터에 의해 지워질 값 : {}",
				 * mainPageDTO.getDiet_code()); iter.remove(); } }
				 */
...
// ****** DTO를 그냥 객체화 해서 써도 되나??
		...
		while (iter.hasNext()) {
			...
			while (iterReply.hasNext()) {
				...
				if (mainPageDTO.getCafe_idx() == mainPageReplyDTO.getCafe_idx()) {
					...
					mainPageDTO.setReply_mem_nick(mainPageReplyDTO.getMem_nick());
				}
			}
	...
	/*
	 * public HashMap<String, MainPageDTO> searchPageSort(ArrayList<MainPageDTO>
	 * lastResult, int category) { logger.info("값 갯수 : {}", lastResult.size());
	 * logger.info("카테고리 분류 : {} ", category);
	 * 
	 * switch(category) { case 1:{ // 좋아요 순
	 * lastResult.stream().sorted(Comparator.comparing(MainPageDTO::getCafe_likecnt)
	 * ).collect(Collectors.toList()); logger.info("여기 잘 탔다"); break; } case 2:{ //
	 * 조회 순
	 * lastResult.stream().sorted(Comparator.comparing(MainPageDTO::getCafe_hit)).
	 * collect(Collectors.toList()); break; } case 3:{ // 별점 순
	 * lastResult.stream().sorted(Comparator.comparing(MainPageDTO::
	 * getCafe_ratestaravg)).collect(Collectors.toList()); break; } }
	 * 
	 * for (MainPageDTO mainPageDTO : lastResult) { logger.info("식당명 : {}",
	 * mainPageDTO.getCafe_title()); logger.info("좋아요 : {}",
